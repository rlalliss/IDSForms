...
import { clearCustomerDraft, defaultCustomer, readCustomerDraft, writeCustomerDraft } from '../customerStorage';
...
export default function Forms() {
  ...
  useEffect(() => {
    try {
      const flagKey = 'idsforms.customer.cleared';
      const alreadyCleared = typeof window !== 'undefined' ? window.sessionStorage.getItem(flagKey) : '1';
      if (!alreadyCleared) {
        clearCustomerDraft();
        setCustomer(readCustomerDraft());
  ...
  function handleCustomerClear() {
    const reset = { ...defaultCustomer };
    setCustomer(reset);
    try {
      clearCustomerDraft();
      setSaveMessage('Customer details cleared.');
      setTimeout(() => setSaveMessage(''), 3500);
    } catch {
      setSaveMessage('');
    }
  ...
  const heroContent = useMemo(() => (
    <div className="forms-hero forms-hero--single">
      <form className="forms-hero__form" onSubmit={handleCustomerSave}>
        ...
        <div className="forms-hero__actions" style={{ display: 'none' }}>
          <button type="button" className="btn btn--ghost btn--icon" onClick={handleCustomerClear}>
